...
- PythonAnywhere: sign up (free tier), create an API token, then deploy with one command: `make deploy-pa`. The script creates the web app and a bash console, pauses once for a one-time browser click to initialize the console (PA won't accept API commands on a console until it has been opened once), then clones the repo, builds the venv, installs deps, uploads the `.env` and WSGI shim, configures the web app, and reloads. Webhook registration is automatic — with `WEBHOOK_URL` set, the bot registers itself with Telegram on every boot.
